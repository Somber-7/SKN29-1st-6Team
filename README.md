...
### 4. 프로젝트 결과
 **신차 등록의 리드 타임 확인** 
- 연료별(경유) 및 차종별(승합/화물) 차량 등록 데이터에서 공통적으로 유가 변동 후 3개월 뒤에 양의 상관관계가 정점에 달함.
- 이는 차량 구매를 결정하고 실제 출고 및 행정상 등록이 완료되기까지 시장에 평균 3개월의 물리적 시차가 존재함을 의미.

 **타겟 시장별 유가 탄력성 차이 확인** 
- **민감군(상용/물류 목적):** 화물, 승합, 경유, 사업용 차량은 유가 변동에 뚜렷한 상관성을 보임.
- 단기적(3개월)으로는 구매 결정을 진행하지만, 고유가가 장기화(5~6개월)되면 유지비 부담으로 인해 등록이 감소하는 경향을 보임.

- **둔감군(일반/승용 목적):** 비사업용 및 승용차는 유가 변동폭과 관계없이 일정한 등록 흐름을 유지.
- 경제 활동의 필수재로서 유가 외의 개인 소득, 금리 등 다른 변수의 영향이 더 큼을 시사.

 **친환경 차량의 유가 독립성 확인** 
- 전기차 및 하이브리드 차량은 유가 상승에 비례하여 수요가 증가할 것이라는 예상과 달리, 유가 변동과의 뚜렷한 양의 상관관계가 나타나지 않음.
- 친환경차 시장의 수요가 연료비 절감 효과보다는 정부 보조금, 충전 인프라 보급률, 신차 출시 등 외부 환경 및 정책적 요인에 의해 주도되고 있음을 확인.