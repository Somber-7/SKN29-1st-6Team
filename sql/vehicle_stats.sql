...
CREATE TABLE IF NOT EXISTS TBL_COMMON_CODE (
    ...
    UNIQUE KEY UQ_COMMON_CODE (CODE)    -- 코드값은 그룹 간 전역 유일 → 자식 테이블 단일 컬럼 FK 참조 허용
) ENGINE=InnoDB
...
CREATE TABLE IF NOT EXISTS TBL_FUEL_REG_STAT (
    ...
    CONSTRAINT FK_FUELSTAT_FUEL   FOREIGN KEY (FUEL_CD)   REFERENCES TBL_COMMON_CODE (CODE),
    CONSTRAINT FK_FUELSTAT_TYPE   FOREIGN KEY (TYPE_CD)   REFERENCES TBL_COMMON_CODE (CODE),
    CONSTRAINT FK_FUELSTAT_USAGE  FOREIGN KEY (USAGE_CD)  REFERENCES TBL_COMMON_CODE (CODE),
    CONSTRAINT FK_FUELSTAT_REGION FOREIGN KEY (REGION_CD) REFERENCES TBL_COMMON_CODE (CODE)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci
...
CREATE TABLE IF NOT EXISTS TBL_FUEL_PRICE (
    ...
    CONSTRAINT FK_FUELPRICE_FUEL   FOREIGN KEY (FUEL_CD)   REFERENCES TBL_COMMON_CODE (CODE),
    CONSTRAINT FK_FUELPRICE_REGION FOREIGN KEY (REGION_CD) REFERENCES TBL_COMMON_CODE (CODE)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci
...
-- ============================================================
-- 기존 테이블 ALTER (테이블·데이터 이미 존재하는 경우 실행)
-- 순서: 부모 → 자식 순으로 적용
-- ============================================================

-- ------------------------------------------------------------
-- 1. TBL_COMMON_CODE — CODE 컬럼에 UNIQUE KEY 추가
--    복합 PK (CODE_GROUP, CODE) 는 유지하면서
--    자식 테이블의 단일 컬럼 FK 참조를 허용
-- ------------------------------------------------------------
ALTER TABLE TBL_COMMON_CODE
    ADD CONSTRAINT UQ_COMMON_CODE UNIQUE (CODE);


-- ------------------------------------------------------------
-- 2. TBL_FUEL_REG_STAT — 코드 컬럼 4개에 FK 추가
-- ------------------------------------------------------------
ALTER TABLE TBL_FUEL_REG_STAT
    ADD CONSTRAINT FK_FUELSTAT_FUEL   FOREIGN KEY (FUEL_CD)   REFERENCES TBL_COMMON_CODE (CODE),
    ADD CONSTRAINT FK_FUELSTAT_TYPE   FOREIGN KEY (TYPE_CD)   REFERENCES TBL_COMMON_CODE (CODE),
    ADD CONSTRAINT FK_FUELSTAT_USAGE  FOREIGN KEY (USAGE_CD)  REFERENCES TBL_COMMON_CODE (CODE),
    ADD CONSTRAINT FK_FUELSTAT_REGION FOREIGN KEY (REGION_CD) REFERENCES TBL_COMMON_CODE (CODE);


-- ------------------------------------------------------------
-- 3. TBL_FUEL_PRICE — 코드 컬럼 2개에 FK 추가
-- ------------------------------------------------------------
ALTER TABLE TBL_FUEL_PRICE
    ADD CONSTRAINT FK_FUELPRICE_FUEL   FOREIGN KEY (FUEL_CD)   REFERENCES TBL_COMMON_CODE (CODE),
    ADD CONSTRAINT FK_FUELPRICE_REGION FOREIGN KEY (REGION_CD) REFERENCES TBL_COMMON_CODE (CODE);
